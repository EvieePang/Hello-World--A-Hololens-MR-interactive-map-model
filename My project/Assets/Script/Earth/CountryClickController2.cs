...
/// </summary>
public class CountryClickController2 : MonoBehaviour
{
    [Header("Country Root")]
    ...
    void Start()
    {
        ...
        foreach (Transform c in countriesParent)
        {
            ...
            // Create label immediately (but hidden)
    ...
    private GameObject CreateLabelObject(GameObject country)
    {
        ...
        // Direction from Earth center → country
        Vector3 normal = (worldCenter - earthTransform.position).normalized;

        // Find the outermost vertex to compute surface radius
        float maxDist = float.MinValue;
        foreach (var v in mesh.vertices)
        {
            Vector3 wv = country.transform.TransformPoint(v);
        ...
        // How far to push the label outside the surface
        float offset = (surfaceRadius - centerRadius) + 0.005f;
        offset = Mathf.Max(offset, 0.005f);

        Vector3 labelPos = worldCenter + normal * offset;

        // Make the label face outward (you can also face camera)
        Quaternion rotation = Quaternion.LookRotation(-normal, cameraTransform.up);
        // If you want label to always face camera, remove ^ and enable this instead:
        // Quaternion rotation = Quaternion.LookRotation(labelPos - cameraTransform.position);

        // ----- Instantiate label -----
        GameObject label = Instantiate(labelPrefab, labelPos, rotation, country.transform);
        ...
        // Initial state is hidden — LOD system will activate it
        label.SetActive(false);
    ...
    void UpdateLabelLOD()
    {
        ...
        foreach (var kvp in countryLabels)
        {
            ...
            // ----- Compute "area" from mesh bounds -----
            float area = country.GetComponent<MeshFilter>().sharedMesh.bounds.size.sqrMagnitude;
            area = Mathf.Max(area, 0.0001f);

            // ----- LOD threshold -----
            // Larger countries → smaller threshold → appear earlier
            // Smaller countries → larger threshold → appear later
            float K = 0.12f;                  // master sensitivity parameter
            float threshold = K / area;       // when scale ≈ threshold → fully visible
            ...
            // Enable/Disable to avoid rendering cost for invisible labels
